...
export class Agent {
  ...
  private buildExtendedPrompt(): string {
    return [
      ...
      "Gmail (find/draft/label/archive/delete email, get attachments), Google Contacts (find/create/update contacts,",
      "groups, photos).",
      ...
      "",
      "**IMPORTANT — Contact lookup workflow:**",
      "When the user asks to contact someone by name (send a text, call, email, etc.), ALWAYS look up",
      "their details in Google Contacts via Zapier first. Never ask the user for a phone number or email",
      "if you can look it up. Search your available Zapier tools for contact-related tools and use them",
      "to find the person by name, then proceed with the action (e.g. Twilio skill for SMS).",
      "",
