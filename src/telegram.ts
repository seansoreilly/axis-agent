    ...
    switch (command) {
      ...
      case "/post": {
        ...
        const prompt = [
          ...
          "For each photo, optimize exposure, saturation, and crop for Facebook.",
          `Use mode '${photos.length === 1 ? "single" : "multi"}' (single = 4:5 portrait 1080x1350, multi = 1:1 square 1080x1080).`,
          "Run for each photo:",
          `node /home/ubuntu/agent/.claude/skills/facebook/scripts/optimize_photo.mjs --input INPUT_PATH --output /tmp/fb_post_N.jpg --mode ${photos.length === 1 ? "single" : "multi"}`,
          "The script auto-adjusts brightness and saturation, and smart-crops using face/saliency detection.",
          "Check the JSON output for adjustments made and report them to the user in Step 5.",
          "Save optimized versions as /tmp/fb_post_N.jpg (where N is 1, 2, 3...)",
          "",
