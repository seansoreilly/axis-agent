...
`;
  ...
  describe("runOnce", () => {
    ...
    it("returns error when file exists but is unreadable", async () => {
      readFileSpy.mockImplementation(() => {
  ...
  describe("start/stop", () => {
    ...
    it("start() clears existing interval to prevent duplicates", async () => {
      ...
      const service = new HeartbeatService(defaultOpts());
      service.start();
      service.start(); // double start — should not leak

      await vi.advanceTimersByTimeAsync(30 * 60 * 1000);
      // Should only fire once, not twice (leaked timer would cause 2 calls)
      expect(mockRunAgent).toHaveBeenCalledTimes(1);
