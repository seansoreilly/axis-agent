...
describe("CircuitBreaker", () => {
  ...
  it("half-open: only allows a single probe request", async () => {
    const { CircuitBreaker, CircuitOpenError } = await import("./circuit-breaker.js");
    const breaker = new CircuitBreaker({ name: "test", failureThreshold: 2, resetTimeoutMs: 5000 });
    ...
    // First call is the probe — make it hang (never resolves)
    let probeResolve!: (v: string) => void;
    const probePromise = breaker.execute(() => new Promise<string>((r) => { probeResolve = r; }));

    // Second concurrent call should be rejected
    await expect(breaker.execute(() => Promise.resolve("should not run"))).rejects.toThrow(CircuitOpenError);
